...
import { Activity, Bug, AlertTriangle, Droplets, Wind, Heart, Cloud, CircleDot, Circle, BarChart3, Waves, Flame, Microscope, Stethoscope } from 'lucide-react';
...
export default function DetectionSpectrum() {
  const pathologies = [
    ...
    { name: "Effusion pleurale",  description: "Liquide pleural",        icon: Droplets },
    { name: "Atélectasie",        description: "Collapsus alvéolaire",   icon: Wind },
    { name: "Infiltration",       description: "Opacités diffuses",      icon: Cloud },
    { name: "Masse",              description: "Lésions > 3 cm",        icon: CircleDot },
    { name: "Nodule",             description: "Lésions < 3 cm",        icon: Circle },
    { name: "Fibrose",            description: "Cicatrisation",          icon: BarChart3 },
    { name: "BPCO",               description: "Obstruction chronique",  icon: Waves },
    { name: "Asthme sévère",      description: "Bronchospasme aigu",     icon: Flame },
    { name: "Cancer bronchique",  description: "Tumeur maligne",         icon: Microscope },
    { name: "Bronchite",          description: "Inflammation bronchique",icon: Stethoscope },
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-(--t2) max-w-2xl mx-auto">
            Notre algorithme est entraîné sur plus de 2 millions de clichés pour identifier{' '}
            <span className="font-semibold text-blue-600">14 pathologies</span> pulmonaires
            avec une fiabilité inégalée.
          </p>
        </motion.div>

        {/* Grille 7 × 2 = 14 cartes */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.1 }}
          className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-5"
        >
