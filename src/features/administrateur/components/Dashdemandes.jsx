...
import { MOCK, mapMedecin, elapsedStr } from "../api/demandesData";
...
export default function DashDemandes({ dark }) {
  ...
  useEffect(() => {
    getDemandes()
      .then(data => setItems(Array.isArray(data) ? data.map(mapMedecin) : MOCK))
      .catch(() => setItems(MOCK));
  }, []);
  ...
  return (
    <Card dark={dark} className="p-5">
      ...
      <div className="flex flex-col gap-2.5">
        {preview.length === 0 ? (
          <p className="text-center text-[13px] py-4" style={{ color: txt.subtle }}>
            Aucune demande en attente
          </p>
        ) : preview.map(d => (
          <div key={d.id} className="flex items-center gap-3 p-3.5 rounded-xl" style={{ background: surface.bg }}>
            {d.photo_url
      ...
      <button
        ...
        className="mt-3 w-full py-2.5 rounded-xl text-[13px] font-bold border transition-colors"
